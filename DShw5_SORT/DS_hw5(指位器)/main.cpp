...
int main(){
 ...
 // cout<<"1";
   ...
   //cout<<"1";
	b1_list = BDM.SORTMONEY(BANK1);//cout<<"1";
	b2_list = BDM.SORTMONEY(BANK2);//cout<<"1";
	b3_list = BDM.SORTMONEY(BANK3);//cout<<"1";
  ...
  /* vector<USER*>::iterator i;
    i=b1_list.begin();
    cout<<(*i)->money<<" "<<(*(i+1))->money<<" "<<(*(i+2))->money<<" "<<(*(i+3))->money<<" "<<(*(i+4))->money;*/
   /* for(int i=0;i<b1_list.size();i++){
	cout<<(b1_list[i])->ID<<" "<<(b1_list[i])->money<<endl;
	}*/
